...
int main(int argc, char **argv) {
    ...
    for (int img_cpt = 0; img_cpt < imagePaths.size(); ++img_cpt) {
        ...
        for (int cpt = 1; cpt <= 101; cpt+=2) {
            std::string imOutPath = imgNewFolderPath + "/" + classString
                                    + "_" + std::to_string(img_cpt) + "_" + std::to_string(cpt) + ".ppm";
    ...
    for (int img_cpt = 0; img_cpt < imagePaths.size(); ++img_cpt) {
        ...
        for (int gsize = 1; gsize <= 8; ++gsize) {
            ...
            for (int i = 0; i < groups.size(); ++i) {
                ...
                std::string imOutPath = imgNewFolderPath + "/" + classString
                                        + "_" + std::to_string(img_cpt) + "_" + std::to_string(gsize) + "_"
                                        + std::to_string(i) + ".ppm";
                std::strcpy(imagePath, imOutPath.c_str());
                imOut.save(imagePath);
            }
